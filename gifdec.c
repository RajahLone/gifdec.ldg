...
gd_open_gif(const char *fname)
{
    ...
    if (bgcolor[0] || bgcolor[1] || bgcolor [2])
        for (i = 0; i < gif->width * gif->height; i++)
            gif->canvas[i] = gif->bgindex; // raj: copy index instead of 3 bytes color
    ...
    for (j = 0; j < gif->fh; j++) {
        for (k = 0; k < gif->fw; k++) {
            ...
            if (!gif->gce.transparency || index != gif->gce.tindex)
                buffer[(i+k)] = index; // raj: index instead color
        }
        i += gif->width;
    }
}
...
dispose(gd_GIF *gif)
{
    ...
    case 2: /* Restore to background color. */
        ...
        for (j = 0; j < gif->fh; j++) {
            for (k = 0; k < gif->fw; k++)
                gif->canvas[(i+k)] = gif->bgindex; // raj: copy index instead of 3 bytes color
            i += gif->width;
        }
        break;
...
{
    memcpy(buffer, gif->canvas, gif->width * gif->height); // raj: copy 2d array colors indexes instead of 3 bytes colored image
